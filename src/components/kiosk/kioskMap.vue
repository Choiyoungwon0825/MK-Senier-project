...
<script>
import kioskSidebar from './kioskSidebar.vue';
...
export default {
  ...
  setup() {
    ...
    const clickedMarker = ref(null); // kioskSidebar에 전달할 마커 정보
    const drawerOpen = ref(false);  // drawer의 열림 상태를 저장할 ref 생성
          ...
          getDocs(storeCollection)
            .then((querySnapshot) => {
              querySnapshot.forEach((doc) => {
                ...
                const stnum = storeData.Stnum; // stores 테이블 Stunum 이랑 reviews 테이블의 storedId랑 비교할 용도
                const latitude = storeData.location.lat;
                const longitude = storeData.location.lng;
                const storeName = storeData.storename;
                const image = storeData.image;
                ...
                window.kakao.maps.event.addListener(marker, 'click', () => {
                  ...
                  // Firestore에서 review 데이터 가져오기
                  const q = query(
                    reviewCollection,
                    where('storeId', '==', String(stnum))
                  );
                  const reviews = [];
                  ...
                  getDocs(q)
                    ...
                    .catch((error) => {
                      console.log(
                        'Firestore에서 review 데이터를 가져오는 중 오류가 발생했습니다.',
                        error
                      );
                    });
                  // 정보를 clickedMarker에 추가
                  drawerOpen.value = true;
                  clickedMarker.value = { storeName, image, reviews};
                });
              });
            })
            .catch((error) => {
              console.log(
                'Firestore store 데이터를 가져오는 중 오류가 발생했습니다.',
                error
              );
            });
        });
